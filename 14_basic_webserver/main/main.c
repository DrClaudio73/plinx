...
const static char http_xls_hdr[] = "HTTP/1.1 200 OK\nContent-type: application/vnd.ms-excel\n\n";
const static char http_off_hml[] = "<meta http-equiv=\"refresh\" content=\"2\"><meta name=\"description\" content=\"Control load from web brwoser\">\n<meta name=\"author\" content=\"Claudio Cattaneo\">\n<meta content=\"width=device-width,initial-scale=1 \"name=viewport><style>div{width:230px;height:300px;position:absolute;top:0;bottom:0;left:0;right:0;margin:auto}</style><div><h1 align=center>Relay is OFF</h1><a href=on.html><img src=on.png></a></div>";
const static char http_on_hml[] = "<meta http-equiv=\"refresh\" content=\"2\"><meta name=\"description\" content=\"Control load from web brwoser\">\n<meta name=\"author\" content=\"Claudio Cattaneo\">\n<meta content=\"width=device-width,initial-scale=1 \"name=viewport><style>div{width:230px;height:300px;position:absolute;top:0;bottom:0;left:0;right:0;margin:auto}</style><div><h1 align=center>Relay is ON</h1><a href=off.html><img src=off.png></a></div>";
...
extern const uint8_t fav_ico_start[] asm("_binary_fav_png_start");
extern const uint8_t fav_ico_end[]   asm("_binary_fav_png_end");
extern const uint8_t xls_xlx_start[] asm("_binary_excel_ods_start");
extern const uint8_t xls_xlx_end[]   asm("_binary_excel_ods_end");
	...
	if (err == ERR_OK) {
		...
		if(first_line) {
			...
			// favicon image
				...
				printf("Sending favicon image...\n");
				netconn_write(conn, http_png_hdr, sizeof(http_png_hdr) - 1, NETCONN_NOCOPY);
				netconn_write(conn, fav_ico_start, fav_ico_end - fav_ico_start, NETCONN_NOCOPY);
			}

			// excel xlsx file
			else if(strstr(first_line, "GET /excel.ods ")) {
				printf("Sending excel file...\n");
				netconn_write(conn, http_xls_hdr, sizeof(http_xls_hdr) - 1, NETCONN_NOCOPY);
				netconn_write(conn, xls_xlx_start, xls_xlx_end - xls_xlx_start, NETCONN_NOCOPY);
			}
...
void toggle_relay() {
	do {
		vTaskDelay(6000/portTICK_RATE_MS); //allows task to be pre-empted and changes relay status
		relay_status=!relay_status;
		gpio_set_level(CONFIG_RELAY_PIN, relay_status);
		gpio_set_level(2, relay_status);
	} while(1);
}
...
void app_main()
{
    ...
    xTaskCreate(&toggle_relay, "toggle_relay", 2048, NULL, 5, NULL);
}
