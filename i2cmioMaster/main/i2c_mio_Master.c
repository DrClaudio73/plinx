 ...
 *
 */
static esp_err_t i2c_example_master_read_slave(i2c_port_t i2c_num, uint8_t* data_rd, size_t size, uint16_t slave_addr)
{
    if (size == 0) {
        return ESP_OK;
    }
 ...
 *
 */
static esp_err_t i2c_example_master_write_slave(i2c_port_t i2c_num, uint8_t* data_wr, size_t size, uint16_t slave_addr)
{
    i2c_cmd_handle_t cmd = i2c_cmd_link_create();
    i2c_master_start(cmd);
...
static void i2c_test_task(void* arg)
{
    ...
    while (1) {
        ...
        j=(j+1)%2;
		if (j==0){
			slave_addr=ESP_SLAVE1_ADDR;
		} else {
			slave_addr=ESP_SLAVE2_ADDR;
		}

        ret = i2c_example_master_read_slave(I2C_EXAMPLE_MASTER_NUM, data_rd, RW_TEST_LENGTH, slave_addr);
	    ...
	    printf("\r\n%s\r\n",data_rd);
            ...
            printf("%s: Master read slave [%d] error, IO not connected...\n", esp_err_to_name(ret), (j+1));
        }
        ...
        ret = i2c_example_master_write_slave( I2C_EXAMPLE_MASTER_NUM, data_wr, RW_TEST_LENGTH, slave_addr);
        if (ret == ESP_ERR_TIMEOUT) {
            printf("I2C timeout\n");
        } else if (ret == ESP_OK) {
            printf("\r\n*******************\r\n");
            printf("MASTER WRITE TO SLAVE [%d]\n", (j+1));
	    printf("%s",data_wr);            
        //disp_buf(data_wr, RW_TEST_LENGTH);
	    printf("\r\n*******************\n");
        } else {
            printf("Slave[%d] %s: Master write slave error, IO not connected....\n", (j+1), esp_err_to_name(ret));
        }
        vTaskDelay(( DELAY_TIME_BETWEEN_ITEMS_MS * ( task_idx + 1 ) ) / portTICK_RATE_MS);
    } //while(1)
}
